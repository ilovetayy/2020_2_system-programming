...
void *mm_malloc(size_t size) 
{
    ...
    //there is no fit, extend the heap-> MAX(asize, chunksize)
...
{
    if(bp == 0 || get_heap_listp() == 0)//nothing to free or heap initialization X
	return;
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    if(size == 0){//if size is 0, free and return NULL
	mm_free(ptr);
	return NULL;
    }

    if(ptr == NULL)//if oldptr is NULL, malloc
	return mm_malloc(size);

    newptr = mm_malloc(size);

    //if realloc() fails, the originam block is left
    if(!newptr)
	return NULL;
